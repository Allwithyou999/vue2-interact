...
var Vue2InteractDraggable = { render: function () {
  ...
  mounted() {
    ...
    const el = interact(element).draggable({
      onstart: () => {
      ...
      onmove: event => {
        ...
        if ((event.pageX < 38) || (event.pageX > (window.innerWidth - 38))) {

        event.interaction.stop(event)
        // event.interaction.clearTargets()
        // event.interaction.removePointer(event.pointers[0])
        // event.interaction.pointerCancel()
        ...
        //   // this.interactResetCardPosition()
          // event.interaction.pointerCancel(event.interaction.pointers[0], event, event.target, event.target)
        //   // event.interaction.clearTargets()
          // event.interaction.pointerUp(event.interaction.pointers[0], event, event.target, event.target)
        //   // event.interaction.pointerCancel(pointer, event, eventTarget, curEventTarget)
        //   console.log('event', event.interaction)
        //   // const { x: cardPositionX, y: cardPositionY } = this.interactPosition;
        //   // const { interactXThreshold, interactYThreshold } = this;
        //   // this.interactIsAnimating = true;
  ...
  methods: {
    ...
    interactDraggedReset() {
      console.log('RESET')
      // if (this.interactBlockReset) {
        // this.interactBlockRest
        // this.interactResetCardPosition();
    ...
    interactSetEventBusEvents() {
      if (this.interactEventBusEvents) {
        if (this.interactEventBusEvents.draggedReset) {
          InteractEventBus.$on(this.interactEventBusEvents.draggedReset, this.interactDraggedReset);
        }
    ...
    interactUnsetEventBusEvents() {
      if (this.interactEventBusEvents) {
        if (this.interactEventBusEvents.reset) {
          InteractEventBus.$off(this.interactEventBusEvents.reset, this.reset);
        }
        if (this.interactEventBusEvents.draggedDown) {
